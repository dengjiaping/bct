package cn.antke.bct.widget.flowlayout;
...
public class FlowTagLayout extends ViewGroup {
	...
	/**
	 * 有没有默认选中以及默认选中的position
	 * 默认  没有默认选中的child
	 */
	private int position;
	private boolean isSetDefaultChecked;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
			...
			//设置默认选中
			if (isSetDefaultChecked && i == position) {
	...
	/**
	 * @param adapter 像ListView、GridView一样使用FlowLayout
	 */
