...
import cn.antke.bct.home.controller.PlateListActivity;
...
public class SpecialFragment extends BaseFragment implements View.OnClickListener, RefreshRecyclerView.OnRefreshAndLoadMoreListener {
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.rigth_text:
				Intent intent = new Intent(getActivity(), StoreApplyActivity.class);
				startActivity(intent);
				break;
	...
	private class PlateHolder implements Holder<List<PlateEntity>>, OnItemHeightGetListener {
		...
		public void UpdateUI(Context context, int position, List<PlateEntity> data) {
			PlateAdapter plateAdapter = new PlateAdapter(getActivity(), data, SpecialFragment.this);
